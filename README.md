...
- Effects: **solid** (all regions on), **sparkle** (regions randomly flicker on/off), **fade** (same as solid), **sweep** (a glowing worm-shaped light travels along the skeleton of the region, changing radius to match line thickness — see Sweep Effect below)
...
The `sweep` effect animates a glowing worm-shaped light traveling along the skeleton of a region. The circle's radius matches the local thickness of the region (from the distance transform), and it leaves a fading trail behind it. At branch points the direction is chosen randomly with a forward bias.

### Generating a sweep path

The sweep effect requires a precomputed skeleton graph for the region. Generate it with `generate_sweep_path.py`:

```bash
/usr/bin/python3 generate_sweep_path.py <region_id_map.png> <region_id> <output.json> --min-seg 0 --downsample 4
```

**Important**: `--min-seg 0` is required. Short (~1.4 px) edges connect adjacent branch points; pruning them disconnects the graph and the worm stops moving.

**Example** (region 5 in the M1 Scene 4 map):
```bash
/usr/bin/python3 generate_sweep_path.py m1s4_region_id_map.png 5 m1s4_g114_sweep_path.json --min-seg 0 --downsample 4
```

Add `--debug` to save a debug image to `/tmp/sweep_debug.png` showing the skeleton and graph overlay.

### Output format

The JSON contains:
- `mapW`, `mapH`: dimensions of the source region ID map
- `regionId`: the region that was processed
- `nodes`: list of `{id, x, y, r, type}` — skeleton endpoints and branch points
- `edges`: list of `{id, from, to, points, length}` — each `points` entry is `[x, y, r]`

### Assigning to a sequence

In the editor, set the step's effect to **Sweep** and fill in the sweep parameters. Or directly in `config.json` under the sequence step:

```json
{
  "group": "g114",
  "effect": "sweep",
  "sweepPath": "m1s4_g114_sweep_path.json",
  "sweepSpeed": 900,
  "sweepEaseIn": 5000,
  "sweepEaseOut": 600,
  "sweepWormLength": 1500,
  "fadeIn": 500,
  "fadeOut": 1000
}
```

The worm starts at a randomly chosen endpoint, travels forward with a bias against reversals, bounces at dead ends, and fades in/out with the standard `fadeIn`/`fadeOut` timing.
...
Sweep effect parameters (set in the editor UI or directly in `config.json`):

| Parameter | Description |
|---|---|
| `sweepPath` | Path to the sweep path JSON file generated by `generate_sweep_path.py` |
| `sweepSpeed` | Base travel speed in pixels per second along the skeleton |
| `sweepEaseIn` | Duration (ms) over which speed ramps from 0 to full at sequence start |
| `sweepEaseOut` | Duration (ms) over which speed ramps back to 0 at sequence end |
| `sweepWormLength` | Length of the worm trail in milliseconds (how long each lit point fades over) |
...
| `m1s4_g114_sweep_path.json` | Sweep path data for region g114 (orange line) in M1 Scene 4 |
