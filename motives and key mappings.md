# Motives and Key Mappings — Malaqatin Meetings

## Mvt 1
### Scene 1

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key | Comments |
|--------|----------------------------|---------------------------|--------------|--------------------|----------|
| M1     | double whole notes         | ball in center            | Q            | C3 (note 48)       |          |
| M2     | quarter notes              | area around ball          | W            | D3 (note 50)       |          |


### Scene 2

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key         | Comments |
|--------|----------------------------|---------------------------|--------------|----------------------------|----------|
| M1     | double whole notes         | ball in center            | Q            | C3 (note 48)               |          |
| M2     | quarter notes              | area around ball          | W            | D3 (note 50)               |          |
| M3     | 8th notes                  | blue area                 | E            | E4 (note 64); G4 = E4+F4   |          |
| M4     | 16th notes                 | bubbles outside blue area | R            | F4 (note 65); G4 = E4+F4   |          |


### Scene 3

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key         | Comments |
|--------|----------------------------|---------------------------|--------------|----------------------------|----------|
| M1     | double whole notes         | ball in center            | Q            | C3 (note 48)               |          |
| M2     | quarter notes              | area around ball          | W            | D3 (note 50)               |          |
| M3     | 8th notes                  | blue area                 | E            | E4 (note 64); G4 = E4+F4   |          |
| M4     | 16th notes                 | bubbles outside blue area | R            | F4 (note 65); G4 = E4+F4   |          |
| M5     | slides                     | larger groups of flowers  | A            | G3 (note 55)               |          |
| M6     | 2-16th, 2-8th, goes up     | orange lines              | S            | A3 (note 57)               |          |

### Scene 4

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key         | Comments |
|--------|----------------------------|---------------------------|--------------|----------------------------|----------|
| M1     | double whole notes         | ball in center            | Q            | C3 (note 48)               |          |
| M2     | quarter notes              | area around ball          | W            | D3 (note 50)               |          |
| M3     | 8th notes                  | blue area                 | E            | E4 (note 64); G4 = E4+F4   |          |
| M4     | 16th notes                 | bubbles outside blue area | R            | F4 (note 65); G4 = E4+F4   |          |
| M5     | slides                     | larger groups of flowers  | A            | G3 (note 55)               |          |
| M6     | 2-16th, 2-8th, goes up     | orange lines              | S            | A3 (note 57)               |          |
| M7     | tremelo                    | flicker outlined areas    | D            | B4 (note 71)               |          |
| M8     | cello solo                 | major outlined circle     | F            | A4 (note 69)               |          |
| M9     | violin solo                | major outlined circle     | F            | A4 (note 69)               |          |

## Mvt 2

### Scene 1
*R denotes rhythmic, NR not rhythmic*

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key         | Comments |
|--------|----------------------------|---------------------------|--------------|----------------------------|----------|
| M1     | half notes (R)             |                           | Q            | C3 (note 48)               |          |
| M2     | 16th → longer note (NR)    |                           | W            | D3 (note 50)               |          |
| M3     | Pizz chords (R)            |                           | E            | E4 (note 64)               |          |
| M4     | 5+ slurred 16th notes (NR) |                           | R            | F4 (note 65)               |          |
| M5     | 8th to  long note (NR)     |                           | A            | G3 (note 55)               |          |
| M6     | 16th note pattern (NR)     |                           | S            | A3 (note 57)               |          |
| M7     | solo cello (NR)            |                           | D            | G4 (note 67)               |          |
| M8     | solo sax (NR)              |                           | F            | A4 (note 69)               |          |
| M9     | solo violin (NR)           |                           | T            | B4 (note 71)               |          |

## Mvt 3

### Scene 4

| Motive | Pattern                    | Visual                    | Computer Key | Music Keyboard Key         | Comments |
|--------|----------------------------|---------------------------|--------------|----------------------------|----------|
| M1     | drum 8th notes             |                           | Q            | C3 (note 48)               |          |
| M2     | conga/cello 16,16th,8th    |                           | W            | D3 (note 50)               |          |
| M3     | castanets/vln1 16th        |                           | E            | E4 (note 64)               |          |
| M4     | sop sax/vln slurred runs   |                           | R            | F4 (note 65)               |          |
| M5     | slurred pickup gesture     |                           | A            | G3 (note 55)               |          |
| M6     | unslurred 8ths/quarters    |                           | S            | A3 (note 57)               |          |
| M7     | pizz 8th notes             |                           | D            | G4 (note 67)               |          |
| M8     | trem                       |                           | F            | A4 (note 69)               |          |
| M9     | pizz repeated 4ers         |                           | G            | B4 (note 71)               |          |
| M10    | solo sax                   |                           | Y            | C4 (note 71)               |          |
| M11    | solo cello                 |                           | U            | D4 (note 71)               |          |
| M12    | solo vln                   |                           | I            | E5 (note 71)               |          |


## Navigation Controls

| Action              | Computer Key | Music Keyboard Key          | Comments              |
|---------------------|--------------|-----------------------------|-----------------------|
| Next scene          | → arrow      | C4 (note 60) or C5 (note 72) | Pitch wheel above 75% |
| Previous scene      | ← arrow      | B3 (note 59)                | Pitch wheel below 25% |
| Fade in             | F            | Knob 8 above halfway        |                       |
| Fade out            | G            | Knob 8 below halfway        |                       |
| Start performance   | Enter        | Any key / Knob 1 above half | Initial load only     |
